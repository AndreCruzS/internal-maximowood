...
export default function Calculator() {
  ...
  // Boards mode: board count × board length → LF
  const [boardCount, setBoardCount] = useState("");
  const [boardLengthFt, setBoardLengthFt] = useState("");
  ...
  // Promo species use long-form names like "MAXIMO THERMO CLEAR RADIATA"
  // Calculator species use short names like "CLEAR RADIATA"
  // We match by checking if the promo species ends with the selected species (case-insensitive)
  // and profile/size match exactly (case-insensitive)
  const promoMatch = useMemo(() => {
    if (!selectedSpecies || !selectedProfile || !selectedSize) return null;
    const speciesUpper = selectedSpecies.toUpperCase();
    const profileUpper = selectedProfile.toUpperCase();
    const sizeUpper = selectedSize.toUpperCase();
    return PROMO_PRODUCTS.find(p =>
      p.species.toUpperCase().includes(speciesUpper) &&
      p.profile.toUpperCase() === profileUpper &&
      p.nominalSize.toUpperCase().replace(/\s/g, "") === sizeUpper.replace(/\s/g, "")
    ) ?? null;
  }, [selectedSpecies, selectedProfile, selectedSize]);

  // Reset species/size/profile when category changes
  ...
  const handleCalculate = () => {
    ...
    if (inputMode === "boards") {
      ...
      if (!boardCount || isNaN(bc) || bc <= 0) { setError("Please enter a valid number of boards."); return; }
      if (!boardLengthFt || isNaN(bl) || bl <= 0) { setError("Please enter a valid board length."); return; }
    } else if (!quantity || isNaN(parseFloat(quantity)) || parseFloat(quantity) <= 0) {
    ...
    const preFinishOpt = PRE_FINISH_COLOR_OPTIONS.find((o) => o.id === preFinishColorType);
    const preFinishPrice = preFinishOpt?.pricePerLF ?? 2.20;

    let addOnCost = 0;
    if (includeMilling) addOnCost += wastedLF * MILLING_PRICE_PER_LF;
    if (includePreFinishColor) addOnCost += wastedLF * preFinishPrice;
    if (includePreFinishTexture) addOnCost += wastedLF * PRE_FINISH_TEXTURE_PRICE_PER_LF;
    ...
    if (includePreFinishColor && preFinishOpt) {
      addOnBreakdown.push({ label: `Pre-Finish Color: ${preFinishOpt.label} ($${preFinishOpt.pricePerLF.toFixed(2)}/LF)`, amount: wastedLF * preFinishOpt.pricePerLF });
    }
  ...
  const handleAddToCart = () => {
    ...
    const speciesLabel = category === "thermo" ? `THERMO® ${selectedSpecies}` : selectedSpecies;
    // When promo is active, override price per LF and total with the promo EC price
    const effectivePricePerLF = (usePromoPrice && promoMatch)
      ? promoMatch[promoScenario].ec
      : results.pricePerLF;
    const effectiveMaterialCost = effectivePricePerLF * results.wastedLF;
    const promoAddOnNote = (usePromoPrice && promoMatch)
      ? [{ label: `🏷️ Promo: ${promoScenario.charAt(0).toUpperCase() + promoScenario.slice(1)} scenario (was $${results.pricePerLF.toFixed(2)}/LF)`, amount: 0 }]
      : [];
    const newItem: CartItem = {
      ...
      lf: results.wastedLF,
      pricePerLF: effectivePricePerLF,
      total: effectiveMaterialCost,
      addOns: [...promoAddOnNote, ...results.addOnBreakdown],
      lengthType,
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
      ...
      <div className="lg:col-span-2 space-y-4">
        ...
        {/* Category Toggle */}
        <div className="flex gap-2 p-1 rounded-xl" style={{ background: "#F0EDE4", border: "1px solid #E0DDD4" }}>
          <button
            type="button"
            onClick={() => handleCategoryChange("thermo")}
            className="flex-1 py-2.5 rounded-lg text-sm font-bold transition-all"
            style={category === "thermo" ? { background: "#1A1A1A", color: "#C9A227" } : { background: "transparent", color: "#888" }}
          >
            Thermowood
          </button>
          <button
            ...
            className="flex-1 py-2.5 rounded-lg text-sm font-bold transition-all"
            style={category === "hardwood" ? { background: "#1A1A1A", color: "#C9A227" } : { background: "transparent", color: "#888" }}
          >
            Hardwood
          </button>
          <button
            type="button"
            onClick={() => handleCategoryChange("accoya")}
            className="flex-1 py-2.5 rounded-lg text-sm font-bold transition-all"
            style={category === "accoya" ? { background: "#1A1A1A", color: "#C9A227" } : { background: "transparent", color: "#888" }}
          >
            Accoya
          </button>
        </div>
        ...
        <StepCard step={4} title="Quantity">
          <div className="space-y-3">
            <div className="flex gap-2 flex-wrap">
              ...
              <ToggleBtn active={inputMode === "boards"} onClick={() => setInputMode("boards")}>
                🪵 Boards (fence)
              </ToggleBtn>
            </div>
            ...
            {inputMode === "boards" ? (
              <div className="space-y-3">
                <p className="text-xs text-[#888]">
                  Enter the number of boards and the length of each board. Total LF = boards × length.
                </p>
                <div className="grid grid-cols-2 gap-3">
                  <div className="space-y-1.5">
                    <Label className="text-xs font-semibold uppercase tracking-wider text-[#888]">Number of Boards</Label>
                    <Input
                      type="number"
                      min="1"
                      step="1"
                      placeholder="Ex: 120 boards"
                      value={boardCount}
                  ...
                  <div className="space-y-1.5">
                    <Label className="text-xs font-semibold uppercase tracking-wider text-[#888]">Board Length (ft)</Label>
                    <Input
                      type="number"
                      min="1"
                      step="0.5"
                      placeholder="Ex: 8 ft"
                      value={boardLengthFt}
                      onChange={(e) => setBoardLengthFt(e.target.value)}
                      className="h-12 text-lg font-semibold border-[#E0DDD4] focus:ring-[#C9A227] focus:border-[#C9A227]"
                    />
                  </div>
                </div>
                {boardCount && boardLengthFt && !isNaN(parseFloat(boardCount)) && !isNaN(parseFloat(boardLengthFt)) && (
                  <div
        ...
        <StepCard step={6} title="Add-ons" icon={<Wrench className="w-4 h-4" />}>
          <div className="space-y-3">
            ...
            {/* Pre-Finish: Color */}
            <div
              className="flex items-center justify-between p-3 rounded-lg border bg-[#FAFAF7]"
              style={{ borderColor: includePreFinishColor ? GOLD : "#E0DDD4" }}
            >
              <div>
        ...
        <div className="flex gap-3">
          <button
            ...
            disabled={!selectedProduct || (inputMode === "boards" ? (!boardCount || !boardLengthFt) : !quantity)}
        ...
        {/* ── Step 8: Promo Discount — only shown when the selected product has a promo entry ── */}
        {promoMatch && (
          <StepCard step={8} title="Promo Discount" icon={<span>🏷️</span>}>
            <div className="space-y-3">
              <p className="text-xs text-[#666]">
                This product is on the clearance list. Select the pricing scenario to apply a promotional End Customer price.
              </p>
              <div className="grid grid-cols-3 gap-2">
                {([
                  { id: "conservador" as PromoScenario, label: "Conservador", sub: "20% margin", color: "#16a34a" },
                  { id: "moderado" as PromoScenario, label: "Moderado", sub: "15% margin", color: "#d97706" },
                  { id: "agressivo" as PromoScenario, label: "Agressivo", sub: "10% margin", color: "#dc2626" },
                ]).map(s => (
                  <button
                    key={s.id}
                    type="button"
                    onClick={() => setPromoScenario(s.id)}
                    className="py-3 px-2 rounded-xl border-2 text-center transition-all"
                    style={promoScenario === s.id
                      ? { borderColor: s.color, background: `${s.color}15` }
                      : { borderColor: "#E0DDD4", background: "white" }
                    }
                  >
                    <p className="text-sm font-black" style={{ color: promoScenario === s.id ? s.color : "#1A1A1A" }}>{s.label}</p>
                    <p className="text-xs font-medium" style={{ color: promoScenario === s.id ? s.color : "#888" }}>{s.sub}</p>
                  </button>
                ))}
              </div>
              <div className="rounded-xl p-4 border-2" style={{ borderColor: "#7C3AED40", background: "#7C3AED08" }}>
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-xs font-bold uppercase tracking-wider" style={{ color: "#7C3AED" }}>EC Promo Price</p>
                    <p className="text-xs text-[#888] mt-0.5">
                      {promoMatch.status === "Descontinuado" ? "🔴 Descontinuado" : promoMatch.status === "A confirmar" ? "🟡 A confirmar" : "🟢 Em linha"}
                      {" · "}{promoMatch.nominalSize} · RL only
                    </p>
                  </div>
                  <p className="text-2xl font-black" style={{ color: "#7C3AED" }}>
                    ${promoMatch[promoScenario].ec.toFixed(2)}/LF
                  </p>
                </div>
                <p className="text-xs text-[#888] mt-2">
                  Standard EC price: ${selectedProduct ? getPrice(selectedProduct, "RL").toFixed(2) : "—"}/LF
                  {selectedProduct && promoMatch[promoScenario].ec < getPrice(selectedProduct, "RL") && (
                    <span className="ml-2 font-bold" style={{ color: "#dc2626" }}>
                      −${(getPrice(selectedProduct, "RL") - promoMatch[promoScenario].ec).toFixed(2)} discount
                    </span>
                  )}
                </p>
              </div>
              ...
              <div
                ...
                style={usePromoPrice
                  ? { borderColor: "#7C3AED", background: "#7C3AED10" }
                  : { borderColor: "#E0DDD4", background: "white" }
                }
                onClick={() => setUsePromoPrice(v => !v)}
              >
                <div>
                  <p className="text-sm font-black" style={{ color: usePromoPrice ? "#7C3AED" : "#1A1A1A" }}>
                    Apply promo price to quote
                  </p>
                  <p className="text-xs" style={{ color: usePromoPrice ? "#7C3AED" : "#888" }}>
                    {usePromoPrice
                      ? `✓ Quote will use $${promoMatch[promoScenario].ec.toFixed(2)}/LF instead of standard price`
                      : "Toggle on to use this promo price when adding to quote"}
                  </p>
                </div>
                <div
                  className="w-10 h-6 rounded-full transition-all flex items-center px-1"
                  style={{ background: usePromoPrice ? "#7C3AED" : "#E0DDD4" }}
                >
                  <div
                    className="w-4 h-4 rounded-full bg-white shadow transition-all"
                    style={{ transform: usePromoPrice ? "translateX(16px)" : "translateX(0)" }}
                  />
                </div>
              </div>
            </div>
