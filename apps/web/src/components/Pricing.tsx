...
import { Search, RefreshCw, Tag, DollarSign, Tag as TagIcon } from "lucide-react";
import { PROMO_PRODUCTS, type PromoScenario } from "@/lib/products";
...
export default function Pricing() {
  const { data: rows, isLoading, error, refetch } = usePricing();

  const [search, setSearch] = useState("");
  const [selectedCategory, setSelectedCategory] = useState<string>("ALL");
  const [selectedTier, setSelectedTier] = useState<PriceTier>("distributor");
  const [promoScenarioView, setPromoScenarioView] = useState<PromoScenario>("conservador");
    ...
    <div className="space-y-6">
      ...
      <div className="space-y-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-lg font-black text-[#1A1A1A]" style={{ fontFamily: "'Anybody', sans-serif" }}>Clearance / Promo Pricing</h2>
            <p className="text-xs text-gray-500">End Customer prices only · Random Lengths · 3 pricing scenarios</p>
          </div>
        </div>
        ...
        <div className="flex gap-2 p-1 rounded-xl bg-gray-100 w-fit">
          {([
            ...
            <button
              key={s.id}
              onClick={() => setPromoScenarioView(s.id)}
              className="px-4 py-2 rounded-lg text-sm font-bold transition-all"
              style={promoScenarioView === s.id
                ? { background: s.color, color: "white" }
                : { background: "transparent", color: "#555" }
              }
            >
              {s.label} <span className="text-xs opacity-70">({s.sub})</span>
            </button>
          ))}
        </div>
        ...
        <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-gray-100 bg-gray-50">
                  <th className="text-left px-5 py-3 text-xs font-bold text-gray-500 uppercase tracking-wider">Species</th>
                  <th className="text-left px-4 py-3 text-xs font-bold text-gray-500 uppercase tracking-wider">Profile</th>
                  <th className="text-left px-4 py-3 text-xs font-bold text-gray-500 uppercase tracking-wider">Size</th>
                  <th className="text-left px-4 py-3 text-xs font-bold text-gray-500 uppercase tracking-wider">Status</th>
                  <th className="text-right px-5 py-3 text-xs font-bold uppercase tracking-wider" style={{ color: "#7C3AED" }}>EC $/LF</th>
                </tr>
              </thead>
              <tbody>
                {PROMO_PRODUCTS.map((p, idx) => {
                  const price = p[promoScenarioView].ec;
                  return (
                    <tr key={p.id} className={`border-b border-gray-50 hover:bg-purple-50/30 transition-colors ${idx === PROMO_PRODUCTS.length - 1 ? "border-b-0" : ""}`}>
                      <td className="px-5 py-3 font-semibold text-gray-800 text-xs">{p.species}</td>
                      <td className="px-4 py-3 text-gray-600 text-xs">{p.profile}</td>
                      <td className="px-4 py-3 text-gray-500 text-xs">{p.nominalSize}</td>
                      <td className="px-4 py-3">
                        <span
                          className="text-[10px] font-bold px-2 py-0.5 rounded-full"
                          style={{
                            background: p.status === "Descontinuado" ? "#fee2e2" : p.status === "A confirmar" ? "#fef3c7" : "#dcfce7",
                            color: p.status === "Descontinuado" ? "#dc2626" : p.status === "A confirmar" ? "#d97706" : "#16a34a",
                          }}
                        >
                          {p.status}
                        </span>
                      ...
                      <td className="px-5 py-3 text-right">
                        {price > 0 ? (
                          <span className="font-black text-base" style={{ color: "#7C3AED", fontFamily: "'Anybody', sans-serif" }}>
                            ${price.toFixed(2)}
                          </span>
                        ) : (
                          <span className="text-gray-300 text-xs">—</span>
