...
| `apps/web/drizzle/migrations/0001_drop_inventory_tables.sql` (+ meta journal) | New migration generated by `drizzle-kit generate` after schema edit; drops `inventory` and `inventory_sync_log`. Output dir is `./drizzle/migrations` per `drizzle.config.ts`. (Note: there are legacy `0000-0002_*.sql` files at `drizzle/` from a prior MySQL-flavoured iteration — those are unused; the active migration set is under `drizzle/migrations/`.) |
