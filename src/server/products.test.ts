...
describe("add-ons calculation", () => {
  ...
  it("pre-finish color options have correct prices", () => {
    expect(PRE_FINISH_COLOR_OPTIONS.find(o => o.id === "regular")?.pricePerLF).toBe(2.20);
    expect(PRE_FINISH_COLOR_OPTIONS.find(o => o.id === "regular_ca")?.pricePerLF).toBe(2.20);
    expect(PRE_FINISH_COLOR_OPTIONS.find(o => o.id === "fluted")?.pricePerLF).toBe(2.40);
    expect(PRE_FINISH_COLOR_OPTIONS.find(o => o.id === "special")?.pricePerLF).toBe(2.70);
  });

  it("pre-finish color regular adds $2.20/LF", () => {
  ...
  it("pre-finish color regular_ca adds $2.20/LF", () => {
    const cost = calculateAddOnCost(100, { milling: false, preFinishColor: true, preFinishColorType: "regular_ca", preFinishTexture: false });
    expect(cost).toBeCloseTo(220.00);
  });

  it("pre-finish color fluted adds $2.40/LF", () => {
  ...
  it("pre-finish color special adds $2.70/LF", () => {
    const cost = calculateAddOnCost(100, { milling: false, preFinishColor: true, preFinishColorType: "special", preFinishTexture: false });
    expect(cost).toBeCloseTo(270.00);
  });

  it("pre-finish texture adds $0.50/LF independently", () => {
    expect(PRE_FINISH_TEXTURE_PRICE_PER_LF).toBe(0.50);
  ...
  it("pre-finish color + texture can be combined", () => {
    ...
    expect(cost).toBeCloseTo(290.00); // 240 + 50
  });
  ...
  it("milling + pre-finish color + texture all combined", () => {
    ...
    expect(cost).toBeCloseTo(370.00); // 100 + 220 + 50
  });
